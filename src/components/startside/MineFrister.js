...
const MineFrister = () => {
    ...
    return (
        <div className="mine-frister">
            <div className="top">
                <h4>Over fristen<div className="frist-count">0</div></h4>
                <h4>Nærmer seg frist<div className="frist-count">3</div></h4>
            </div>
            <div className="main">
                {frister.map(({title, deadline, type, comment}) => (
                    <ListElement key={title}>
                        <div className={`container ${comment ? "has-comment" : ""}`}>
                            ...
                            {comment ? <img src={MessageIcon} alt="Message icon" /> : null}
                        </div>
                    </ListElement>
                ))}
            </div>
